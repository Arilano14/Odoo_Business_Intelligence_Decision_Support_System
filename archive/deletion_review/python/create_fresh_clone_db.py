...
try:
    ...
    # Terminate active sessions on primary DB to allow TEMPLATE clone
    cursor.execute(f"""
        SELECT pg_terminate_backend(pid)
        FROM pg_stat_activity
        WHERE datname = '{primary_db}' AND pid <> pg_backend_pid();
    """)
    print("Terminated idle sessions on primary DB.")
    ...
    if exists:
        cursor.execute(f"""
            SELECT pg_terminate_backend(pid)
            FROM pg_stat_activity
            WHERE datname = '{clone_db}' AND pid <> pg_backend_pid();
        """)
        cursor.execute(f"DROP DATABASE \"{clone_db}\"")

    print(f"Creating fresh isolated clone database '{clone_db}'...")
    cursor.execute(f"CREATE DATABASE \"{clone_db}\" WITH TEMPLATE \"{primary_db}\" OWNER openpg;")
    print("Fresh clone database created successfully!")

    cursor.close()
    conn.close()

    # Verify connection to fresh clone DB
    ...
    print(f"Verified Fresh Clone DB Row Counts: SOs={so_cnt}, POs={po_cnt}")
    cursor_clone.close()
    conn_clone.close()

    print("GATE 2B CLONE CREATION: 100% PASSED!")
