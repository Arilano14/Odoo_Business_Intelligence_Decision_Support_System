"""
OBIDSS Dashboard Artifact Generator (Audited Version)
======================================================
Generates versioned JSON payloads for all 5 OBIDSS dashboards using exact
audited baseline models, measures, and positional formula syntax (#field).

Audited Truth Baselines (FY 2026, company_id = 2):
- Confirmed Sales Value (Untaxed): Rp 17,552,025,691.43 (sale.order)
- Confirmed Purchase Value (Untaxed): Rp 30,088,422,406.50 (purchase.order)
- Confirmed SO Count: 677 (sale.order)
- Confirmed PO Count: 225 (purchase.order)
- Portfolio SKUs with Stock: 230 (stock.quant positive internal)
...
def build_executive():
    ...
    j["pivots"]["5"] = make_pivot("5", "Monthly Sales Trend", "sale.order",
        [COMPANY_FILTER, ["state", "=", "sale"], ["date_order", ">=", "2026-01-01"], ["date_order", "<", "2027-01-01"]],
        ["amount_untaxed"], columns=["date_order:month"])
    j["pivots"]["6"] = make_pivot("6", "Monthly Purchase Trend", "purchase.order",
        [COMPANY_FILTER, ["state", "=", "purchase"], ["date_order", ">=", "2026-01-01"], ["date_order", "<", "2027-01-01"]],
        ["amount_untaxed"], columns=["date_order:month"])
    j["pivots"]["7"] = make_pivot("7", "Portfolio SKUs with Stock", "stock.quant",
        [COMPANY_FILTER, ["location_id.usage", "=", "internal"], ["quantity", ">", 0]],
        ["__count"])

    dashboard_cells = {
        "A1": {"content": "Executive Operations Dashboard — FY 2026", "style": 1},
    ...
    for i in range(1, 13):
        r = i + 1
        data_cells[f"A{r}"] = {"content": f"=PIVOT.HEADER(5,\"#date_order:month\",{i})"}
        data_cells[f"B{r}"] = {"content": f"=PIVOT.VALUE(5,\"amount_untaxed\",\"#date_order:month\",{i})"}
        data_cells[f"C{r}"] = {"content": f"=PIVOT.VALUE(6,\"amount_untaxed\",\"#date_order:month\",{i})"}
...
def build_sales():
    ...
    j["pivots"]["2"] = make_pivot("2", "Top Categories", "sale.report",
        [COMPANY_FILTER, ["state", "=", "sale"], ["date", ">=", "2026-01-01"], ["date", "<", "2027-01-01"]],
    ...
    for i in range(1, 13):
        r = i + 1
        data_cells[f"H{r}"] = {"content": f"=PIVOT.HEADER(5,\"#date_order:month\",{i})"}
        data_cells[f"I{r}"] = {"content": f"=PIVOT.VALUE(5,\"amount_untaxed\",\"#date_order:month\",{i})"}
...
def build_purchase():
    ...
    for i in range(1, 13):
        ...
        data_cells[f"G{r}"] = {"content": f"=PIVOT.VALUE(5,\"amount_untaxed\",\"#date_order:month\",{i})"}
    ...
    j["sheets"] = [
        {
            ...
            "figures": [
                ...
                make_chart("chart_products", "bar", "Top Products Purchased", "Data!D2:D6", "Data!C2:C6", 540, 160),
...
def build_inventory():
    ...
    j["pivots"]["4"] = make_pivot("4", "Inventory Adjustments", "stock.move",
        [COMPANY_FILTER, ["state", "=", "done"], ["location_id.usage", "=", "internal"], ["location_dest_id.usage", "=", "inventory"]],
        ["quantity"])
    ...
    dashboard_cells = {
        ...
        "A5": {"content": "Internal Transfers (Done)", "style": 2},
        "C5": {"content": "=PIVOT.VALUE(3,\"quantity\")", "style": 6},
        "A6": {"content": "Inventory Adjustments (Done)", "style": 2},
    ...
    j["sheets"] = [
        {
            ...
            "cells": dashboard_cells,
            "figures": [
                make_chart("chart_onhand_categ", "bar", "On-Hand Quantity by Product Category", "Data!B2:B8", "Data!A2:A8", 20, 240, w=1020),
            ]
        },
    ...
    j["sheets"] = [
        {
            ...
            "cells": dashboard_cells,
            "figures": [
                make_chart("chart_so_states", "pie", "SO Status Distribution", "Data!B2:B5", "Data!A2:A5", 20, 80, w=480, h=240),
                make_chart("chart_po_states", "pie", "PO Status Distribution", "Data!E2:E6", "Data!D2:D6", 520, 80, w=480, h=240),
            ]
        },
...
# ============================================================
# MAIN GENERATION & SAVING TO MODULE FILES
# ============================================================
...
for artifact_name, (xml_id, builder_fn) in dashboards.items():
    print(f"\nBuilding artifact: {artifact_name} ({xml_id})")
    payload = builder_fn()
    ...
    results[artifact_name] = {"xml_id": xml_id, "hash": json_hash, "file": filepath}

    # Also update DB record via ORM
    ...
    if dashboard_record:
        ...
        print(f"  Updated DB record ID: {dashboard_record.id}")

env.cr.commit()
print("\n[STRUCTURAL VALIDATION PASS] All 5 dashboard artifacts generated and saved successfully!")
